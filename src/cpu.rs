...
impl<'a> Cpu<'a> {
    ...
    pub fn tick(&mut self) {
        ...
        match instr {
            ...
            0x02 => ops::write_value_to_memory_at_address(&mut *self.mem, self.reg.a.read(), self.reg.b.read(), self.reg.c.read()), // LD (BC), A
            ...
            0x06 => ops::ld_u8(&mut self.reg.b, ops::u8_immediate(&mut *self.mem, &mut self.reg.pc)), // LD B, n
            0x07 => ops::rotate_left_with_carry(&mut self.reg.a, &mut self.reg.f), // RLC A
            0x08 => ops::write_u16_immediate_address(&mut *self.mem, &mut self.reg.pc, sp), // LD (nn), SP
            0x09 => ops::add_register_pair_to_register_pair(&mut self.reg.h, &mut self.reg.l, b, c, &mut self.reg.f), // ADD HL, BC
            0x0A => ops::ld_from_reg_pair_as_address(&mut *self.mem, &mut self.reg.a, b, c), // LD A, (BC)
            ...
            0x0E => ops::ld_u8(&mut self.reg.c, ops::u8_immediate(&mut *self.mem, &mut self.reg.pc)), // LD C, n
            ...
            0x11 => ops::ld_u16_immediate(&mut *self.mem, &mut self.reg.pc, &mut self.reg.d, &mut self.reg.e), // LD DE, nn
            0x12 => ops::write_value_to_memory_at_address(&mut *self.mem, a, d, e), // LD (DE), A
            ...
            0x16 => ops::ld_u8(&mut self.reg.d, ops::u8_immediate(&mut *self.mem, &mut self.reg.pc)), // LD D, n
            0x17 => ops::rotate_left_with_carry(&mut self.reg.a, &mut self.reg.f), // RL A
            0x18 => ops::jump_by_signed_immediate(&mut *self.mem, &mut self.reg.pc), // JR n
            0x19 => ops::add_register_pair_to_register_pair(&mut self.reg.h, &mut self.reg.l, d, e, &mut self.reg.f), // Add HL, DE
            0x1A => ops::ld_from_reg_pair_as_address(&mut *self.mem, &mut self.reg.a, d, e), // LD A, (DE)
            ...
            0x20 => ops::relative_jmp_by_signed_immediate_if_not_flag(&mut *self.mem, &mut self.reg.pc, &self.reg.f, ZeroFlag), // JR NZ, n
            0x21 => ops::ld_u16_immediate(&mut *self.mem, &mut self.reg.pc, &mut self.reg.h, &mut self.reg.l), // LD HL, nn
            0x22 => ops::write_value_to_memory_at_address_and_increment_register(&mut *self.mem, self.reg.a.read(), &mut self.reg.h, &mut self.reg.l), // LDI (HL), A
            ...
            0x26 => ops::ld_u8(&mut self.reg.h, ops::u8_immediate(&mut *self.mem, &mut self.reg.pc)), // LD H, n
            0x27 => error!("DAA instruction not implemented and is being used"),
            0x28 => ops::relative_jmp_by_signed_immediate_if_flag(&mut *self.mem, &mut self.reg.pc, &self.reg.f, ZeroFlag), // JR Z, n
            0x29 => ops::add_register_pair_to_register_pair(&mut self.reg.h, &mut self.reg.l, h, l, &mut self.reg.f), // ADD HL, HL 
            0x2A => ops::ld_from_address_pointed_to_by_register_pair_and_increment_register_pair(&mut *self.mem, &mut self.reg.a, &mut self.reg.h, &mut self.reg.l), // LDI A, HL
            ...
            0x2E => ops::ld_u8(&mut self.reg.l, ops::u8_immediate(&mut *self.mem, &mut self.reg.pc)), // LD L, d8
            0x2F => ops::complement(&mut self.reg.a, &mut self.reg.f), // CPL 

            0x30 => ops::relative_jmp_by_signed_immediate_if_not_flag(&mut *self.mem, &mut self.reg.pc, &self.reg.f, CarryFlag), // JR NC, n
            0x31 => ops::ld_next_two_bytes_into_reg(&mut *self.mem, &mut self.reg.pc, &mut self.reg.sp), // LD SP, nn
            0x32 => ops::write_value_to_memory_at_address_and_decrement_register(&mut *self.mem, self.reg.a.read(), &mut self.reg.h, &mut self.reg.l), // LDI (HL), A
            0x33 => self.reg.sp.increment(), // INC SP 
            0x34 => ops::increment_value_at_address(&mut *self.mem, self.reg.h.read(), self.reg.l.read(), &mut self.reg.f),
            0x35 => ops::decrement_value_at_address(&mut *self.mem, self.reg.h.read(), self.reg.l.read(), &mut self.reg.f),
            0x36 => ops::ld_u8_immediate_into_address(&mut *self.mem, &mut self.reg.pc, self.reg.h.read(), self.reg.l.read()),
            0x37 => ops::set_flag(&mut self.reg.f, CarryFlag),
            0x38 => ops::relative_jmp_by_signed_immediate_if_flag(&mut *self.mem, &mut self.reg.pc, &mut self.reg.f, CarryFlag),
            ...
            0x3A => ops::ld_from_address_pointed_to_by_register_pair_and_decrement_register_pair(&mut *self.mem, &mut self.reg.a, &mut self.reg.h, &mut self.reg.l), // LDD A, HL
            ...
            0x3E => ops::ld_u8(&mut self.reg.a, ops::u8_immediate(&mut *self.mem, &mut self.reg.pc)), // LD A, d8
            ...
            0x46 => ops::ld_from_reg_pair_as_address(&mut *self.mem, &mut self.reg.b, h, l), // LD B, (HL)
            ...
            0x4E => ops::ld_from_reg_pair_as_address(&mut *self.mem, &mut self.reg.b, h, l), // LD C, (HL) 
            ...
            0x56 => ops::ld_from_reg_pair_as_address(&mut *self.mem, &mut self.reg.d, h, l), // LD D, (HL)
            ...
            0x5E => ops::ld_from_reg_pair_as_address(&mut *self.mem, &mut self.reg.b, h, l), // LD E, (HL) 
            ...
            0x66 => ops::ld_from_reg_pair_as_address(&mut *self.mem, &mut self.reg.h, h, l), // LD H, (HL)
            ...
            0x6E => ops::ld_from_reg_pair_as_address(&mut *self.mem, &mut self.reg.b, h, l), // LD L, (HL) 
            0x6F => ops::ld_u8(&mut self.reg.l, a), // LD L, A          
            0x70 => ops::write_value_to_memory_at_address(&mut *self.mem, b, h, l), // LD (HL), B
            0x71 => ops::write_value_to_memory_at_address(&mut *self.mem, c, h, l), // LD (HL), C
            0x72 => ops::write_value_to_memory_at_address(&mut *self.mem, d, h, l), // LD (HL), D
            ...
            0x7E => ops::ld_from_reg_pair_as_address(&mut *self.mem, &mut self.reg.b, h, l), // LD A, (HL) 
            ...
            0x86 => ops::add_value_at_address(&mut *self.mem, &mut self.reg.a, h, l, &mut self.reg.f), // ADD A, (HL)
            ...
            0x8E => ops::adc_value_at_address(&mut *self.mem, &mut self.reg.a, pack_u16(h, l), &mut self.reg.f), // ADC A, (HL)
            ...
            0x96 => ops::sub_value_at_address(&mut *self.mem, &mut self.reg.a, pack_u16(h, l), &mut self.reg.f), // SUB A, (HL)
            ...
            0x9E => ops::sbc_value_at_address(&mut *self.mem, &mut self.reg.a, pack_u16(h, l), &mut self.reg.f),  // SBC A, (HL)
            ...
            0xA6 => ops::and_value_at_address(&mut *self.mem, &mut self.reg.a, pack_u16(h, l), &mut self.reg.f), // AND A, (HL)
            ...
            0xAE => ops::xor_value_at_address(&mut *self.mem, &mut self.reg.a, pack_u16(h, l), &mut self.reg.f), // XOR A, (HL)
            ...
            0xB6 => ops::or_value_at_address(&mut *self.mem, &mut self.reg.a, pack_u16(h, l), &mut self.reg.f), // OR A, (HL)
            ...
            0xBE => ops::compare_value_at_address(&mut *self.mem, &mut self.reg.a, pack_u16(h, l), &mut self.reg.f), // CP A, (HL)
            0xBF => ops::compare(&mut self.reg.a, a, &mut self.reg.f), // CP A, A
            0xC0 => ops::ret(&mut *self.mem, &mut self.reg.pc, &mut self.reg.sp, not_zero), // RET NZ
            0xC1 => ops::pop(&mut *self.mem, &mut self.reg.sp, &mut self.reg.b, &mut self.reg.c), // POP BC
            0xC2 => ops::jp_u16_immediate_if_true(&mut *self.mem, &mut self.reg.pc, not_zero), // JP NZ, nn
            0xC3 => ops::jp_u16_immediate(&mut *self.mem, &mut self.reg.pc), // JP nn
            0xC4 => ops::call_immediate_if_true(&mut *self.mem, &mut self.reg.pc, &mut self.reg.sp, not_zero), // CALL NZ, nn
            0xC5 => ops::push(&mut *self.mem, &mut self.reg.sp, pack_u16(b, c)), // PUSH BC 
            0xC6 => ops::add_u8_immediate(&mut *self.mem, &mut self.reg.pc, &mut self.reg.a, &mut self.reg.f, false), // ADD A, n
            0xC7 => ops::call(&mut *self.mem, &mut self.reg.pc, &mut self.reg.sp, 0x00), // RST 0
            0xC8 => ops::ret(&mut *self.mem, &mut self.reg.pc, &mut self.reg.sp, zero), // RET Z
            0xC9 => ops::ret(&mut *self.mem, &mut self.reg.pc, &mut self.reg.sp, true), // RET
            0xCA => ops::jp_u16_immediate_if_true(&mut *self.mem, &mut self.reg.pc, zero), // JP Z, nn
            0xCB => self.extended_opcodes(),
            0xCC => ops::call_immediate_if_true(&mut *self.mem, &mut self.reg.pc, &mut self.reg.sp, zero), // CALL Z, nn
            0xCD => ops::call_immediate_if_true(&mut *self.mem, &mut self.reg.pc, &mut self.reg.sp, true), // CALL nn
            0xCE => ops::add_u8_immediate(&mut *self.mem, &mut self.reg.pc, &mut self.reg.a, &mut self.reg.f, true), // ADC A, n
            0xCF => ops::call(&mut *self.mem, &mut self.reg.pc, &mut self.reg.sp, 0x08), // RST 8
            0xD0 => ops::ret(&mut *self.mem, &mut self.reg.pc, &mut self.reg.sp, no_carry), // RET NC
            0xD1 => ops::pop(&mut *self.mem, &mut self.reg.sp, &mut self.reg.d, &mut self.reg.e), // POP DE 
            0xD2 => ops::jp_u16_immediate_if_true(&mut *self.mem, &mut self.reg.pc, no_carry), // JP NC, nn
            0xD3 => error!("0xD3 should never be executed"), 
            0xD4 => ops::call_immediate_if_true(&mut *self.mem, &mut self.reg.pc, &mut self.reg.sp, no_carry), // CALL NC, nn
            0xD5 => ops::push(&mut *self.mem, &mut self.reg.sp, pack_u16(d, e)), // PUSH DE 
            0xD6 => ops::sub_u8_immediate(&mut *self.mem, &mut self.reg.pc, &mut self.reg.a, &mut self.reg.f, false), // SUB A, n
            0xD7 => ops::call(&mut *self.mem, &mut self.reg.pc, &mut self.reg.sp, 0x10), // RST 10
            0xD8 => ops::ret(&mut *self.mem, &mut self.reg.pc, &mut self.reg.sp, carry), // RET C
            0xD9 => ops::reti(&mut *self.mem, &mut self.reg.pc, &mut self.reg.sp, &mut self.reg.ime), // RETI
            0xDA => ops::jp_u16_immediate_if_true(&mut *self.mem, &mut self.reg.pc, carry), // JP C, nn
            0xDB => error!("0xDB should never be executed"),
            0xDC => ops::call_immediate_if_true(&mut *self.mem, &mut self.reg.pc, &mut self.reg.sp, carry), // CALL C, nn
            ...
            0xE5 => ops::push(&mut *self.mem, &mut self.reg.sp, pack_u16(h, l)), // PUSH HL 
            0xE6 => ops::and(&mut self.reg.a, ops::u8_immediate(&mut *self.mem, &mut self.reg.pc), &mut self.reg.f), // AND A, n
            0xE7 => ops::call(&mut *self.mem, &mut self.reg.pc, &mut self.reg.sp, 0x20), // RST 20
            0xE8 => error!("Not Implemented"), // ADD SP, (signed)n
            0xE9 => ops::jp(&mut self.reg.pc, pack_u16(h,l)), // JP (HL)
            0xEA => ops::write_value_to_u16_immediate(&mut *self.mem, &mut self.reg.pc, a), // LD (nn), A 
    ...
    fn extended_opcodes(&mut self) {
        ...
        match instr {
            ...
            0x06 => ops::rotate_left_with_carry_at_address(&mut *self.mem, pack_u16(h, l), &mut self.reg.f), // RLC (HL) 
            ...
            0x0E => ops::rotate_right_with_carry_at_address(&mut *self.mem, pack_u16(h, l), &mut self.reg.f), // RRC (HL) 
            ...
            0x16 => ops::rotate_left_at_address(&mut *self.mem, pack_u16(h, l), &mut self.reg.f), // RL (HL) 
            ...
            0x1E => ops::rotate_right_at_address(&mut *self.mem, pack_u16(h, l), &mut self.reg.f), // RR (HL) 
            0x1F => ops::rotate_right(&mut self.reg.a, &mut self.reg.f), // RR E

            0x20 => ops::sla(&mut self.reg.b, &mut self.reg.f), // SLA B
            0x21 => ops::sla(&mut self.reg.c, &mut self.reg.f), // SLA C
            0x22 => ops::sla(&mut self.reg.d, &mut self.reg.f), // SLA D
            0x23 => ops::sla(&mut self.reg.e, &mut self.reg.f), // SLA E
            0x24 => ops::sla(&mut self.reg.h, &mut self.reg.f), // SLA B
            0x25 => ops::sla(&mut self.reg.l, &mut self.reg.f), // SLA C
            0x26 => ops::sla_at_address(&mut *self.mem, pack_u16(h, l), &mut self.reg.f), // SLA (HL) 
            0x27 => ops::sla(&mut self.reg.a, &mut self.reg.f), // SLA E
            0x28 => ops::sra(&mut self.reg.b, &mut self.reg.f), // SRA B
            0x29 => ops::sra(&mut self.reg.c, &mut self.reg.f), // SRA C
            0x2A => ops::sra(&mut self.reg.d, &mut self.reg.f), // SRA D
            0x2B => ops::sra(&mut self.reg.e, &mut self.reg.f), // SRA E
            0x2C => ops::sra(&mut self.reg.h, &mut self.reg.f), // SRA B
            0x2D => ops::sra(&mut self.reg.l, &mut self.reg.f), // SRA C
            0x2E => ops::sra_at_address(&mut *self.mem, pack_u16(h, l), &mut self.reg.f), // SRA (HL) 
            0x2F => ops::sra(&mut self.reg.a, &mut self.reg.f), // SRA E
            
            0x30 => ops::swap(&mut self.reg.b, &mut self.reg.f), // SLA B
            0x31 => ops::swap(&mut self.reg.c, &mut self.reg.f), // SLA C
            0x32 => ops::swap(&mut self.reg.d, &mut self.reg.f), // SLA D
            0x33 => ops::swap(&mut self.reg.e, &mut self.reg.f), // SLA E
            0x34 => ops::swap(&mut self.reg.h, &mut self.reg.f), // SLA B
            0x35 => ops::swap(&mut self.reg.l, &mut self.reg.f), // SLA C
            0x36 => ops::swap_at_address(&mut *self.mem, pack_u16(h, l), &mut self.reg.f), // SLA (HL) 
            0x37 => ops::swap(&mut self.reg.a, &mut self.reg.f), // SLA E
            0x38 => ops::srl(&mut self.reg.b, &mut self.reg.f), // SRA B
            0x39 => ops::srl(&mut self.reg.c, &mut self.reg.f), // SRA C
            0x3A => ops::srl(&mut self.reg.d, &mut self.reg.f), // SRA D
            0x3B => ops::srl(&mut self.reg.e, &mut self.reg.f), // SRA E
            0x3C => ops::srl(&mut self.reg.h, &mut self.reg.f), // SRA B
            0x3D => ops::srl(&mut self.reg.l, &mut self.reg.f), // SRA C
            0x3E => ops::srl_at_address(&mut *self.mem, pack_u16(h, l), &mut self.reg.f), // SRA (HL) 
            0x3F => ops::srl(&mut self.reg.a, &mut self.reg.f), // SRA E
            ...
            0x46 => ops::bit(ops::byte_at_address(&mut *self.mem, pack_u16(h,l)), 0, &mut self.reg.f), // BIT 0, (HL)
            ...
            0x4E => ops::bit(ops::byte_at_address(&mut *self.mem, pack_u16(h,l)), 1, &mut self.reg.f), // BIT 1, (HL)
            ...
            0x56 => ops::bit(ops::byte_at_address(&mut *self.mem, pack_u16(h,l)), 2, &mut self.reg.f), // BIT 2, (HL)
            ...
            0x5E => ops::bit(ops::byte_at_address(&mut *self.mem, pack_u16(h,l)), 3, &mut self.reg.f), // BIT 3, (HL)
            ...
            0x66 => ops::bit(ops::byte_at_address(&mut *self.mem, pack_u16(h,l)), 4, &mut self.reg.f), // BIT 4, (HL)
            ...
            0x6E => ops::bit(ops::byte_at_address(&mut *self.mem, pack_u16(h,l)), 5, &mut self.reg.f), // BIT 5, (HL)
            ...
            0x76 => ops::bit(ops::byte_at_address(&mut *self.mem, pack_u16(h,l)), 6, &mut self.reg.f), // BIT 6, (HL)
            ...
            0x7E => ops::bit(ops::byte_at_address(&mut *self.mem, pack_u16(h,l)), 7, &mut self.reg.f), // BIT 7, (HL)
            ...
            0x86 => ops::res_at_addr(&mut *self.mem, pack_u16(h,l), 0), // RES 0, (HL)
            ...
            0x8E => ops::res_at_addr(&mut *self.mem, pack_u16(h,l), 1), // RES 1, (HL)
            ...
            0x96 => ops::res_at_addr(&mut *self.mem, pack_u16(h,l), 2), // RES 2, (HL)
            ...
            0x9E => ops::res_at_addr(&mut *self.mem, pack_u16(h,l), 3), // RES 3, (HL)
            ...
            0xA6 => ops::res_at_addr(&mut *self.mem, pack_u16(h,l), 4), // RES 4, (HL)
            ...
            0xAE => ops::res_at_addr(&mut *self.mem, pack_u16(h,l), 5), // RES 5, (HL)
            ...
            0xB6 => ops::res_at_addr(&mut *self.mem, pack_u16(h,l), 6), // RES 6, (HL)
            ...
            0xBE => ops::res_at_addr(&mut *self.mem, pack_u16(h,l), 7), // RES 7, (HL)
            ...
            0xC6 => ops::set_at_addr(&mut *self.mem, pack_u16(h,l), 0), // SET 0, (HL)
            ...
            0xCE => ops::set_at_addr(&mut *self.mem, pack_u16(h,l), 1), // SET 1, (HL)
            ...
            0xD6 => ops::set_at_addr(&mut *self.mem, pack_u16(h,l), 2), // SET 2, (HL)
            ...
            0xDE => ops::set_at_addr(&mut *self.mem, pack_u16(h,l), 3), // SET 3, (HL)
            ...
            0xE6 => ops::set_at_addr(&mut *self.mem, pack_u16(h,l), 4), // SET 4, (HL)
            ...
            0xEE => ops::set_at_addr(&mut *self.mem, pack_u16(h,l), 5), // SET 5, (HL)
            ...
            0xF6 => ops::set_at_addr(&mut *self.mem, pack_u16(h,l), 6), // SET 6, (HL)
            ...
            0xFE => ops::set_at_addr(&mut *self.mem, pack_u16(h,l), 7), // SET 7, (HL)
